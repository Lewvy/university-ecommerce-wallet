	...
	const handleVerify = async (e: React.FormEvent) => {
		...
		try {
			...
			if (verifyResponse.ok) {
				...
				// Auto-login logic (this part was already correct)
				try {
					...
					if (loginResponse.ok) {
						...
						setTimeout(() => onVerificationComplete(), 3000)
					}
				} catch (loginError) {
				...
				setError(verifyData.message || "Invalid verification token. Please check the code and try again.")
			}
		} catch (error) {
